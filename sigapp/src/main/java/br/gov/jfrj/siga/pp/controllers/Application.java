...
//			render(resultado,dtt);
	...
	public static void agendamento_atualiza(String cod_sala, String data_ag, String hora_ag) {
		...
		if (objUsuario != null) {
			// Pega o usu·rio do sistema, e, busca os locais(salas) daquele
			// forum onde ele est·.
			Locais objSala = Locais.AR.find("cod_forum='" + objUsuario.forumFk.getCod_forum() + "' and cod_local='" + cod_sala + "'").first(); // isso n„o d· erro no caso de retorno vazio?
...
//			render(frm_processo_ag, listAgendamentos, listPeritos);
	...
	public static void agendamento_sala_lista(String frm_cod_local, String frm_data_ag){
		...
		if (objUsuario != null) {
			// Pega o usu·rio do sistema, e, busca os locais(salas) daquele
			// forum onde ele est·.
			listSalas = ((List) Locais.AR.find("forumFk='" + objUsuario.forumFk.getCod_forum() + "' order by ordem_apresentacao ").fetch()); // isso n„o d· erro no caso de retorno vazio.
