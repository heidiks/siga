package br.gov.jfrj.siga.pp.controllers;
...
//		String matriculaSessao = cadastrante().getMatricula().toString();
	    String matriculaSessao = "";
		UsuarioForum objUsuario = UsuarioForum.AR.find("matricula_usu =" + matriculaSessao).first();
		if (objUsuario != null) {
			try {
				List<Locais> lstLocais = Locais.AR.find("forumFk=" + objUsuario.getForumFk().cod_forum	+ "order by ordem_apresentacao ").fetch();
				Foruns objForum = Foruns.AR.find("cod_forum=" + objUsuario.getForumFk().cod_forum).first();
...
//				render(listLocais);
				...
				try {
					Object auxLocal = Locais.AR.findById(cod_local);
...
//					render(listLocais);
	...
	public static void sala_delete(String cod_sala) {
		...
		if (!cod_sala.isEmpty()) {
			try {
				resposta = "Ok.";
//				Locais.AR.delete("from Locais where cod_local='" + cod_sala + "'");
			} catch (Exception e) {
				e.printStackTrace();
	...
	public static void foruns_listar(Integer cod_forum, String descricao_forum) {
		...
		if (objUsuario != null) {
			...
			if (cod_forum == -1 && descricao_forum.isEmpty()) {
				listForuns = Foruns.AR.findAll();
//				render(listForuns);
			} else if (cod_forum >= 0) {
				Object auxForum = Foruns.AR.findById(cod_forum);
				if (auxForum != null) {
					listForuns.add((Foruns) auxForum);
				}
//				render(listForuns);
			} else if (!descricao_forum.isEmpty()) {
				List<Foruns> resultado = ContextoPersistencia
						.em()
						.createQuery(
								"from Foruns where descricao_forum like '"
										+ descricao_forum + "%'")
	...
	public static void agendamento_incluir_ajax(String fixo_perito_juizo) {
		...
		if (objUsuario != null) {
			...
			listSalas = (List) Locais.AR.find(
					"cod_forum='" + objUsuario.getForumFk().cod_forum
	...
	public static void agendamento_excluir(String frm_data_ag) {
		...
		if (objUsuario != null) {
			...
			if (frm_data_ag != null) {
				...
				listAgendamentos = Agendamentos.AR.find("data_ag = to_date('" + frm_data_ag + "','dd-mm-yy') order by hora_ag , cod_local")
						.fetch();
				...
				if (listAgendamentos.size() != 0) {
					...
					for (Integer i = 0; i < listAgendamentos.size(); i++) {
						...
						for (Integer ii = 0; ii < listLocais.size(); ii++) {
							...
							if (listAgendamentos.get(i).localFk.getCod_local() == listLocais
									.get(ii).getCod_local()) {
...
//			render(resultado,dtt);
	...
	public static void agendamento_atualiza(String cod_sala, String data_ag, String hora_ag) {
		...
		if (objUsuario != null) {
			// Pega o usu·rio do sistema, e, busca os locais(salas) daquele
			// forum onde ele est·.
			Locais objSala = Locais.AR.find("cod_forum='" + objUsuario.getForumFk().cod_forum + "' and cod_local='" + cod_sala + "'").first(); // isso n„o d· erro no caso de retorno vazio?
...
//			render(resultado,data_ag);
	...
	public static void agendadas_hoje() {
		...
		if (objUsuario != null) {
			...
			List<Locais> listaDeSalas = Locais.AR.find("forumFk="+objUsuario.getForumFk().cod_forum).fetch();
...
//			render(frm_processo_ag, listAgendamentos, listPeritos);
	...
	public static void agendamento_sala_lista(String frm_cod_local, String frm_data_ag){
		...
		if (objUsuario != null) {
			// Pega o usu·rio do sistema, e, busca os locais(salas) daquele
			// forum onde ele est·.
			listSalas = ((List) Locais.AR.find("forumFk='" + objUsuario.getForumFk().cod_forum + "' order by ordem_apresentacao ").fetch()); // isso n„o d· erro no caso de retorno vazio.
	...
	public static void usuario_atualiza(String paramCodForum) throws Exception {
		...
		if (objUsuario != null) {
			...
			if (paramCodForum != null && !paramCodForum.isEmpty()) {
				...
				paramCodForum = Integer.toString(objUsuario.getForumFk().cod_forum);
